...
class Hicpo 
{
	...
	*/
	...
	function _check_load_script_css()
	{
		...
		if ( isset( $_GET['orderby'] ) || strstr( $_SERVER['REQUEST_URI'], 'action=edit' ) || strstr( $_SERVER['REQUEST_URI'], WP_ADMIN_DIR_NAME.'/post-new.php' ) ) return false;
		
		if ( !empty( $objects ) ) {
			if ( isset( $_GET['post_type'] ) && !isset( $_GET['taxonomy'] ) && in_array( $_GET['post_type'], $objects ) ) { // if page or custom post types
				$active = true;
			}
			if ( !isset( $_GET['post_type'] ) && strstr( $_SERVER['REQUEST_URI'], WP_ADMIN_DIR_NAME.'/edit.php' ) && in_array( 'post', $objects ) ) { // if post
				$active = true;
			}
		}
