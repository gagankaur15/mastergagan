			...
			if ( $revision = get_post( $revision_id ) ) {
				...
				if ( agp_user_can( $cap_name, $revision->post_parent, '', array( 'skip_revision_allowance' => true ) ) ) {
					...
					if ( in_array( $revision->post_status, array( 'pending' ) ) ) {
						$link = wp_nonce_url( WP_ADMIN_DIR_NAME.'/' . "admin.php?page=rvy-revisions&amp;revision=$revision_id&amp;diff=false&amp;action=approve", "approve-post_$published_post_id|$revision_id" );
					
					} elseif ( in_array( $revision->post_status, array( 'inherit', 'future' ) ) ) {
						$link = wp_nonce_url( WP_ADMIN_DIR_NAME.'/' . "admin.php?page=rvy-revisions&amp;revision=$revision_id&amp;diff=false&amp;action=restore", "restore-post_$published_post_id|$revision_id" );
