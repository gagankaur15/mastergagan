...
include_once( ABSPATH . '/'.WP_ADMIN_DIR_NAME.'/includes/plugin.php' );
...
<?php	
	...
	function build_menu() {
		if ( strpos( $_SERVER['REQUEST_URI'], WP_ADMIN_DIR_NAME.'/network/' ) )
			return;
	...
	function act_impose_pending_rev() {
		...
		//wp_die( $msg, __('Pending Revision Created', 'revisionary'), array( 'response' => 500 ) );

//wp_die( wp_json_encode( array( 'status' => 'error', 'message' => $msg ) ) );
//$this->my_admin_notice($msg);
add_action('admin_head','addAlert');
wp_redirect( admin_url($manage_link->uri) );
exit;

wp_send_json( array( 'success' => true, 'data' => $msg ) ); // for success
