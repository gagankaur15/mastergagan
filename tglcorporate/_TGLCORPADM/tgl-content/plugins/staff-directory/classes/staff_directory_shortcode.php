...
class StaffDirectoryShortcode {
  static function register_shortcode() {
    ...
    add_shortcode('director-list', array('StaffDirectoryShortcode', 'shortcode_director'));
  }
  ...
  static function shortcode_director($params) {
    extract(shortcode_atts(array(
  		'id' => '',
  		'orderby' => '',
  		'order' => ''
    ...
    if(isset($params['template'])) {
  	...
  	// get all staff
  	$param = "id=$id&orderby=$orderby&order=$order";
  	return StaffDirectoryShortcode::show_director_directory($param, $template);
  }
  ...
  static function show_director_directory($param = null, $template = NULL){
  	parse_str($param);
  	global $wpdb;
...
EOT;
  ...
  static function html_for_custom_template($template_slug, $wp_query) {
    ...
    while($wp_query->have_posts()) {
      ...
      $accepted_single_tags = array("[id]","[name]", "[photo_url]", "[bio]", "[category]");
  		$replace_single_values = array($staff_id,$staff_name, $photo_url, $staff_bio, $staff_category);
