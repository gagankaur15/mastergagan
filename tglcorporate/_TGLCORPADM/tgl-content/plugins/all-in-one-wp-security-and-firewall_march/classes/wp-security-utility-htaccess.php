...
class AIOWPSecurity_Utility_Htaccess
{
    ...
    static function getrules_enable_brute_force_prevention()
    {
        ...
        if ($aio_wp_security->configs->get_value('aiowps_enable_brute_force_attack_prevention') == '1') {
            ...
            $rules .= 'RewriteCond %{REQUEST_URI} ('.WP_ADMIN_DIR_NAME.'|wp-login)' . PHP_EOL;// If URI contains wp-admin or wp-login
            if ($aio_wp_security->configs->get_value('aiowps_brute_force_attack_prevention_ajax_exception') == '1') {
                $rules .= 'RewriteCond %{REQUEST_URI} !('.WP_ADMIN_DIR_NAME.'/admin-ajax.php)' . PHP_EOL; // To allow ajax requests through
            }
