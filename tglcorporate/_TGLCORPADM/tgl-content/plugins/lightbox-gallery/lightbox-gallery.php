...
function lightbox_gallery_wp_print_scripts() {
	...
	if ( !is_admin() && $flag ) :
		...
		if ( @file_exists(STYLESHEETPATH.'/lightbox-gallery.js') ) :
			wp_enqueue_script( 'lightbox-gallery', '/'.WP_CONTENT_DIR_NAME.'/themes/' . $template . '/lightbox-gallery.js', array('jquery'), '', $in_footer );
		else :
...
function lightbox_gallery_print_path_header() {
	...
	if ( get_option('home') != get_option('siteurl') || ($url != get_option('siteurl') && !is_multisite()) ) :
		...
		echo '// <![CDATA['."\n";
		if ( $options['global_settings']['lightbox_gallery_loading_type'] == 'highslide' ) :
			echo 'var graphicsDir = "'.get_option('siteurl').'/'.WP_CONTENT_DIR_NAME.'/plugins/lightbox-gallery/graphics/";'."\n";
		elseif ( $options['global_settings']['lightbox_gallery_loading_type'] == 'lightbox' ) :
			echo 'var lightbox_path = "'.get_option('siteurl').'/'.WP_CONTENT_DIR_NAME.'/plugins/lightbox-gallery/";'."\n";
		endif;
		echo '// ]]>'."\n";
		echo '</script>'."\n";
	endif;
