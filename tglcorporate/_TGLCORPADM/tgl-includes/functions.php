...
function do_robots() {
	...
	if ( '0' == $public ) {
		...
		$output .= "Disallow: $path/".WP_ADMIN_DIR_NAME."/\n";
		$output .= "Allow: $path/".WP_ADMIN_DIR_NAME."/admin-ajax.php\n";
	}
...
function wp_guess_url() {
	if ( defined('WP_SITEURL') && '' != WP_SITEURL ) {
		...
		if ( strpos( $_SERVER['REQUEST_URI'], WP_ADMIN_DIR_NAME ) !== false || strpos( $_SERVER['REQUEST_URI'], 'wp-login.php' ) !== false ) {
			$path = preg_replace( '#/('.WP_ADMIN_DIR_NAME.'/.*|wp-login.php)#i', '', $_SERVER['REQUEST_URI'] );
